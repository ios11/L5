...
@interface PRKKeyedStore ()
@property (nonatomic) dispatch_queue_t syncQueue;
@property (nonatomic) NSDictionary *store;
@property (nonatomic) id<PRKKeyedStoreBackend> storeBackend;
@end

@implementation PRKKeyedStore
@dynamic allKeys;

POSRX_DEADLY_INITIALIZER(init)

- (instancetype)initWithBackend:(id<PRKKeyedStoreBackend>)backend
                          error:(NSError **)error {
    POSRX_CHECK(backend);
    if (self = [super init]) {
...
- (BOOL)removeAllObjects:(NSError **)error {
    ...
    PRKAssignError(error, storeError);
    return result;
}
...
- (NSDictionary *)p_loadStore:(NSError **)error {
    @try {
        ...
        if (![store isKindOfClass:[NSDictionary class]]) {
            PRKAssignError(error, [NSError prk_internalErrorWithFormat:@"Unexpected type of the store: %@", store]);
            return [NSDictionary new];
        }
        return [store mutableCopy];
    } @catch (NSException *exception) {
        PRKAssignError(error, [NSError prk_systemErrorWithFormat:exception.reason]);
        return [NSDictionary new];
    }
}
