//
//  PRKKeychainKeyedStoreBackend.m
//  PhotoPark
//
...
@interface PRKKeychainKeyedStoreBackend ()
@property (nonatomic, readonly) NSString *dataKey;
@property (nonatomic, readonly) NSString *service;
@property (nonatomic, readonly) NSString *accessGroup;
@end

@implementation PRKKeychainKeyedStoreBackend
...
- (BOOL)saveData:(NSData *)data error:(NSError **)error {
    ...
    if (status == errSecSuccess) {
        ...
        if (status != errSecSuccess) {
            PRKAssignError(error, [NSError prk_systemErrorWithFormat:@"Failed to update data in keychain, status=%@", @(status)]);
            return NO;
        }
        return YES;
    }
    if (status == errSecItemNotFound) {
        ...
        if (status != errSecSuccess) {
            PRKAssignError(error, [NSError prk_systemErrorWithFormat:@"Failed to insert data in keychain, status=%@", @(status)]);
            return NO;
        }
        return YES;
    }
    PRKAssignError(error, [NSError prk_systemErrorWithFormat:@"Failed to lookup data in keychain, status=%@", @(status)]);
    return NO;
}
...
- (BOOL)removeData:(NSError **)error {
    ...
    PRKAssignError(error, [NSError prk_systemErrorWithFormat:@"Failed to delete data from keychain, status=%@", @(status)]);
    return NO;
}
