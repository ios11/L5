...
* Domain. Функционал, специфичный для предметной области данного приложения. Здесь происходит модилирование сущностей и
связанныех с ними бизнес-правил. Сущности находятся в папке Entities, а манипулирующие ими сервисы в 
папке Services. Помещая сюда класс, вы должны подумать, а действительно ли данный функционал с сутью
является неотемлемой частью предметной области. Объединив Domain с Infrastructure и Utilities можно
получить SDK для вашего сериса.
...
Все сервисы реализуеют некий протокол. Это позволяет легко делать для них заглушечную реализацию,
чтобы писать юнит-тесты для их клиентов. Загляните в папочку Tests данного проекта, чтобы посмотреть
каким образом пишется юнит-тест для PRKKeyedStore, используя заглушечную реализацию KeyedStoreBackend.
...
же сервисов между собой посредством сигналов делает удобным связывание сервисов, живущих как в одном,
так и в разных планировщиках. Об этом следующий раздел.
...
уметь делать большое, но ограниченное число методов для выполнения запросов. Хорошая - предоставит возможность добавлять
...
из всем извстной библиотеки [AFNetworking](https://github.com/AFNetworking/AFNetworking) и
...
всех картинок из Dropbox необходимо полносью сохранить их в FeedViewController. У меня в Dropbox фоток огромное
